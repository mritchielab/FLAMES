...
def remove_similar_tr(gene_to_transcript, transcript_to_exon, thr=10):
    ...
    print("\tRemoved similar transcripts in gene annotation:",
          str(dup_stat), flush=True)


def test_tr(gene_to_transcript):
    i = 0
    for g in gene_to_transcript:
        print((str(gene_to_transcript[g]) + "\t" +
               str([gene_to_transcript[g]]) + "\t" +
               str(type(gene_to_transcript[g])) + "\n"))
                ...
                if tuple(junctions["junctions"]) in self.junction_dict:
                    ...
                    for j in self.junction_dict:
                        if len(j) == len(junctions["junctions"]):
                            distance_per_key = [
                                abs(it-ij) for it, ij in zip(j, junctions["junctions"])]
...
def group_bam2isoform_multisample(bam_in_list, out_gff3, out_stat, summary_csv, chr_to_blocks, gene_dict, transcript_to_junctions, transcript_dict, fa_f, config, downsample_ratio, raw_gff3=None):
    """
    ...
    for ch in chr_to_blocks:
        ...
        for ith, bl in enumerate(chr_to_blocks[ch]):
            ...
            it_region = itertools.chain.from_iterable(
