...
sc_DTU_analysis <- function(sce, path, min_count=15){
    ...
    if (!missing(sce)) {
      ...
      transcript_count <- sce@assays@data@listData$`Flames Single Cell`
    ...
    SummarizedExperiment::rowData(tr_sce) = DataFrame(tr_anno)
    ...
    SummarizedExperiment::rowData(tr_sce)$gene_id=gsub("\\..*","", SummarizedExperiment::rowData(tr_sce)$gene_id)
    ...
    SummarizedExperiment::rowData(tr_sce)$gene_name = SummarizedExperiment::rowData(tr_sce)$gene_id
    ...
    SummarizedExperiment::colData(tr_sce) = cbind(SummarizedExperiment::colData(tr_sce),DataFrame(cluster_barcode_anno))
    cat(paste(c('Found cluster information for ', length(comm_cells), ' cell(s).\n')))
    ...
    row_meta = row_meta %>% group_by(gene_name) %>% top_n(n = 4, wt = mean) # Consider adding a parameter here to allow user to decide how many to keep?
    ...
    get_rm = function(x){
      ...
      tmp = tmp %>% group_by(gene_name) %>% top_n(n = 2, wt=l)
