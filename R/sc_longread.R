...
get_gene_flat <- function(gene_to_transcript, transcript_to_exon) {
    gene_flat <- callBasilisk(flames_nopysam_env, function(gene_tran, tran_exon) {
        python_path <- system.file("python", package="FLAMES")
...
get_gene_blocks <- function(gene_dict, chr_to_gene, gene_to_transcript) {
    gene_blocks <- callBasilisk(flames_nopysam_env, function(g_dict, chr_gene, gene_tran) {
        python_path <- system.file("python", package="FLAMES")
...
group_bam2isoform <- function(bam_in, out_gff3, out_stat, summary_csv, chr_to_blocks, gene_dict,
                             ...
                             raw_gff3=NULL) {
    callBasilisk(flames_nopysam_env, function(bin, o_gff3, o_stat, summary, chr, gene, trans_junc, trans_dict, fa, conf, dr, raw) {
        python_path <- system.file("python", package="FLAMES")
