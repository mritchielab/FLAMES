...
#'
#' @md
#' @description FLAMES requires demultiplexed fastq file as input, which can be acquired by running \code{xxx}.
...
#' and transcript start/end to the reference transcript were merged with the
#' reference. This process will also collapse isoforms that are likely to be truncated
#' transcripts. Next is the read realignment step (\code{do_read_realign}), where the sequence of each polished transcript was extracted and used as
#' the updated reference. The reads were realigned to this reference by minimap2. The
#' transcripts with only a few full-length aligned reads were discarded.
#' The reads were assigned to transcripts based on both alignment score, fractions of
#' reads aligned and transcript coverage. Reads that cannot be uniquely assigned to
#' transcripts or had low transcript coverage were discarded. The UMI transcript
#' count matrix was generated by collapsing the reads with the same UMI in a similar
