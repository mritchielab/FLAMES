...
#' @export
sc_annotate_umap <- function(sce_20, sce_80, path, gene, n_isoforms = 4, n_pcs = 40, cluster_annotation, dup_bc=NULL, sce_all = NULL, return_sce_all = T){
...
#  }
  ...
  if (length(isoform_sel) == 2) {
    ...
    plot_isoforms_plain <- ggbio::autoplot(isoform_sel, label = TRUE) +  # uncolored isoforms for heatmap annotation
      theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                       panel.background = element_blank(), axis.line = element_line(colour = "black"))
  } else {
    plot_isoforms <- ggbio::autoplot(isoform_sel, label = TRUE) + 
      theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                       panel.background = element_blank(), axis.line = element_line(colour = "black"))
    plot_isoforms_plain  <- plot_isoforms
  ...
  if (file.exists(file.path(path,"cluster_annotation.csv")) || (!missing(cluster_annotation) && file.exists(cluster_annotation)) || !is.null(sce_all$cell_type)) {
    ...
    expr_20 <- expr_20[,!apply(expr_20,2,function(x){any(is.na(x))})]
    expr_all <- expr_all[,!apply(expr_all,2,function(x){any(is.na(x))})]

    cluster_barcode_20 <- cluster_barcode[colnames(expr_20), "groups", drop=FALSE]
    cluster_barcode_all <- cluster_barcode[colnames(expr_all), "groups", drop=FALSE]

    cell_order_20 <- stats::hclust(stats::dist(t(expr_20)))$order
    cell_order_all <- stats::hclust(stats::dist(t(expr_all)))$order
    heatmap <- pheatmap(expr_20[tr_order,cell_order_20],
                        cluster_rows = F, cluster_cols = F, 
                        show_colnames = F, show_rownames = T,
                        annotation_col = cluster_barcode_20,
                        width = 4, height = 3)
    heatmap_impute <- pheatmap(expr_all[tr_order,cell_order_all],
                        cluster_rows = F, cluster_cols = F, 
                        show_colnames = F, show_rownames = T,
                        annotation_col = cluster_barcode_all,
                        width = 4, height = 3)
    heatmap_nolengend <- pheatmap(expr_20[tr_order,cell_order_20],
                        cluster_rows = F, cluster_cols = F, 
                        show_colnames = F, show_rownames = F,
                        annotation_col = cluster_barcode_20,
                        width = 4, height = 3)
    heatmap_nolengend_impute <- pheatmap(expr_all[tr_order,cell_order_all],
                        cluster_rows = F, cluster_cols = F, 
                        show_colnames = F, show_rownames = F,
                        annotation_col = cluster_barcode_all,
                        width = 4, height = 3)
  ...
  outputs <- list(plot_umap = plot_umap,
                  ...
                  plot_expression_umaps_impute = plot_expression_umaps_impute,
                  combined_isoform_plot = combined_isoform_plot,
                  combined_isoform_plot_impute = combined_isoform_plot_impute) 
  ...
  if (!is.null(heatmap)) {
    ...
    outputs[["heatmap_impute"]] <- heatmap_impute
    ...
    outputs[["combined_heatmap_impute"]] <- plot_grid(plot_isoforms_plain@ggplot, as.ggplot(heatmap_nolengend_impute), 
                                               rel_widths = c(1,1.2), align = "hv")
  }
