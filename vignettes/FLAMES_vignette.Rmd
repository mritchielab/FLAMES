...
fastq <- bfc[[names(BiocFileCache::bfcadd(bfc, "Fastq", file.path(file_url, "sc_align2genome.sample.fastq.gz")))]]
...
```{r, eval=TRUE, echo=TRUE}
library(FLAMES)
if (is.character(locate_minimap2_dir())) { # do not run if minimap2 cannot be found
...
## Visulisation
```{r, eval=TRUE, echo=TRUE}
source(system.file("examples/scmixology1.R", package = "FLAMES")) # load scm_lib80, scm_lib80 and scm_lib20
unfiltered <- combine_sce(short_read_large = scm_lib80, short_read_small = scm_lib20, long_read_sce = scm_lib20_transcripts, remove_duplicates = FALSE)
unfiltered <- scuttle::addPerCellQC(unfiltered, use.altexps = FALSE)
qc <- scuttle::quickPerCellQC(colData(unfiltered), batch = unfiltered$Lib_small)
qc$discard[unfiltered$Lib_small & unfiltered$sum < 20000] <- TRUE
qc$discard[(!unfiltered$Lib_small) & unfiltered$sum < 10000] <- TRUE
combined_sce <- unfiltered[, !qc$discard]
plots <- sc_annotate_plots(gene = "ENSG00000108107", sce_all = scuttle::quickPerCellQC(combined_sce))
scater::plotReducedDim(plots[["sce_all"]], dimred = "PCA", colour_by = "Lib_small")
plots[["plot_umap"]]
...
fastq_dir <- file.path(temp_path, "fastq_dir") # the downloaded fastq files need to be in a directory to be merged together
...
sed -i '/"do_/s/true/false/' config_sclr_nanopore_5end.json # set all steps to false
sed -i '/do_genome_alignment/s/false/true/' config_sclr_nanopore_5end.json # set do_genome_alignment to true
srun -c 20 --mem 64GB Rscript flames_pipeline.R && \

sed -i '/"do_/s/true/false/' config_sclr_nanopore_5end.json && \
sed -i '/do_isoform_identification/s/false/true/' config_sclr_nanopore_5end.json && \
srun -c 1 --mem 64GB --ntasks=1 Rscript flames_pipeline.R  && \

sed -i '/"do_/s/true/false/' config_sclr_nanopore_5end.json && \
sed -i '/do_read_realignment/s/false/true/' config_sclr_nanopore_5end.json && \
srun -c 20 --mem 64GB --ntasks=1 Rscript flames_pipeline.R && \

sed -i '/"do_/s/true/false/' config_sclr_nanopore_5end.json && \
sed -i '/do_transcript_quantification/s/false/true/' config_sclr_nanopore_5end.json && \
srun -c 1 --mem 64GB --ntasks=1 Rscript flames_pipeline.R && \
...
```{bash eval=FALSE}
./flames.sh | tee -a bash.log; tail bash.log | mail -s "flames pipeline ended" user@example.com
```
