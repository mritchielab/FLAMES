...
vignette: >
  %\VignetteIndexEntry{bulk_long_windows}
...
annot <- system.file("data/SIRV_isoforms_multi-fasta-annotation_C_170612a.gtf", package="FlamesR")
outdir <- paste(getwd(), "FlamesWindowsOutDir", sep="/")
fastq_dir <- system.file("data/fastq", package="FlamesR")
genome_fa <- system.file("data/SIRV_isoforms_multi-fasta_170612a.fasta", package="FlamesR")
config_file <- system.file("data/SIRV_config.json", package="FlamesR")
downsample_ratio <- 1
...
```
If a FLAMES configuration file doesn't exist, one can be created using user specified values, and loaded into R using the function `FlamesR::create_config()`. For more information on the contents of the FLAMES JSON configuration file, and for the input parameters to `create_config()`, run `?FlamesR::create_config()`.

An example of the default FLAMES config file can be found at `r system.file("data/SIRV_config.json", package="FlamesR")`.
...
If genome alignment is required, this should be performed on a system with access to minimap2. The code required to run this is provided below. After this is executed and resulting data transfered to this workspace, the Windows FLAMES bulk pipeline can proceed. All required variables for this section should be transfered to the system with minimap2, the code executed and finally the resulting file `genome_bam` and the index file `genome_bam.bai` (the name of each of these will differ based on the variable `genome_bam`) returned to the original system and R workspace.
```{r eval=FALSE}
...
```
The next line, which converts the annotation file to a bed12 file should only be run if the config$use_junctions flag is `TRUE`. The example data requires config$use_junctions to be `TRUE`.
```{r eval=FALSE}
...
file.remove(tmp_bam)
```
Again, the next step is only required if config$use_junctions is `TRUE`.
```{r eval=FALSE}
# if config$use_junctions is TRUE
file.remove(tmp_bed)
```
...
# copy the bam files into the outdir
file.copy(system.file("data/align2genome.bam", package="FlamesR"), outdir)
file.copy(system.file("data/align2genome.bam.bai", package="FlamesR"), outdir)
# bring the file genome_bam, prebuilt into this vignette for downstream use.
genome_bam <- paste0(outdir, "/align2genome.bam")
```
After executing this code on a system with access to minimap2, the file `genome_bam` should be returned to the original system, and it's file path loaded into the original R session. `genome_bam` is required for later pipeline steps.
...
file.remove(tmp_bam)
```
The file realign_bam and the index file (realign_bam.bai) should be returned back to the original system, and it's file path loaded into the same R session, as it is required for the final FlamesR step.

```{r eval=TRUE, echo=FALSE, results='hide'}
# move the stored realign2transcript.bam and .bai files to the outdir
# copy the bam files into the outdir
file.copy(system.file("data/realign2transcript.bam", package="FlamesR"), outdir)
file.copy(system.file("data/realign2transcript.bam.bai", package="FlamesR"), outdir)
# bring the file genome_bam, prebuilt into this vignette for downstream use.
genome_bam <- system.file(paste0(outdir, "/realign2transcript.bam"))
```
